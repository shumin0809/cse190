package com.cse190.petcafe;

public class GlobalStrings {
	public static final String LOGTAG = "Pet Cafe";
	public static final String PREFNAME = "TemporaryDataFile";
	
	public static final String FIRST_INSTALL_KEY = "firstInstall";
	
	public static final String USERNAME_CACHE_KEY = "username";
	public static final String FACEBOOK_ID_CACHE_KEY = "facebookID";
	
	public static final String APP_ID = "19697";
	public static final String AUTH_KEY = "eAHV2kwMXrPCTVP";
	public static final String AUTH_SECRET = "MvEEXW-TcaAwnSc";
	
	public static final String USER_LOGIN = "dscormc";
	public static final String USER_PASSWORD = "B5Ri;iFu";
}
